/**
 * Скрипт для обновления цен товаров по артикулам
 */
  ...
  for (const update of priceUpdates) {
    try {
      ...
      if (response.ok && result.success) {
        console.log(`✓ Обновлен артикул ${update.sku}: ${update.price}₽ (было ${update.originalPrice}₽)`);
        updated++;
      } else {
        console.log(`❌ Не найден артикул ${update.sku}`);
        notFound++;
      }
    } catch (error) {
      console.error(`Ошибка обновления артикула ${update.sku}:`, error);
      notFound++;
    }
  }
...
if (typeof window !== 'undefined') {
  ...
  module.exports = { updatePrices, priceUpdates };
}